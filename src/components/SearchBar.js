...
export default function SearchBar({ filterSpaceshipList }) {

  const [spaceshipName, setspaceshipName] = useState('')

  const handleSearch = (e) => {
    e.preventDefault()
    setspaceshipName(e.target.value);
    filterSpaceshipList(e.target.value)
  }
  ...
  return <div>
    <form onSubmit={handleSearch} className="flex flex-row items-center">
      <div className="relative bg-slate-200 p-2 rounded-xl border-8 border-white w-full">
        ...
        <input onChange={handleSearch} value={spaceshipName} className="ml-10 bg-transparent !outline-none" type="text" placeholder="Search..." />
      </div>
      <div>
